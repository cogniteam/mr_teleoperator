...
//	connect(_mrmControllerUi.selectInputButton,		SIGNAL(clicked()), this, SLOT(selectInputClicked()));
	...
	connect(_mrmControllerUi.addControllerButton,    SIGNAL(clicked()), this, SLOT(addControllerClicked()));
}
...
bool MrRqt::setupInputOuput(ros::NodeHandle& node)
{
    ...
    */
    ...
    foreach(string velocityTopic, _outputTopics){
        _mrmControllerUi.controllableRobots->addItem(
                QString::fromStdString(velocityTopic));
    }
...
void MrRqt::addControllerClicked()
{
    QDockWidget* dock = new QDockWidget();

    dock->setAttribute(Qt::WA_DeleteOnClose);

    TeleoperatorInstanceWidget::InputMethod inputMethod = (TeleoperatorInstanceWidget::InputMethod)(_mrmControllerUi.inputMethods->currentIndex());
    string velocityTopic = _mrmControllerUi.controllableRobots->currentText().toStdString();
    string feedbackVelocityTopic = inputMethod == TeleoperatorInstanceWidget::Joystick ? "/joy_cmd_vel" : velocityTopic;

    QWidget* dockContent = new TeleoperatorInstanceWidget(
                _node,
                inputMethod,
                _mrmControllerUi.controllableRobots->currentIndex() + 1,
                velocityTopic,
                feedbackVelocityTopic);

    dock->setWidget(dockContent);
    _mrmControllerUi.horizontalLayoutContainer->addWidget(dock);
}
